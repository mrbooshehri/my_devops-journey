...
> **Note:** You should change `etcd` data directory in `volumes`
> section, and **NOT** in `container spec` section.

# Security

## K8s security primitives

- All access to the hosts must be secure
  - Root access disabled
  - Password based authentication disabled
  - Only ssh-key based authentication to be made available
- If access to the hosts is compromised, everything is compromised

> **Note:** It's a good practice to **harden** the OS first of all.

## Secure kubernetes

- Kube-apiserver is the first line of defence
...
- Ways that you can authenticate to the API server
  - Files - Username and Password (Not recommended, it's in plain text)
  - Files - Username and Token (Not recommended, it's in plain text)
  - Certificates
  - External authentication providers - LDAP
  - Service Account (handled by kubernetes)

Authorization - What can they do?

- Ways you can authorize the user
...
![ca](assets/certificate-ca.png)

**Server certificates**: Deploy on server components to secure the
communications.

![ca](assets/ca-server.png)

**Client certificates**: Just like servers, clients has private and
public keys to make a secure communication.

![ca](assets/ca-clients.png)

Here there is an overview of certs

![ca](assets/all-cets.png)

### Generate Certificate

The first cert needs to generate is Certificate Authority(CA), because
all the other certs will be signed with this cert.

![ca](assets/ca-gen.png)

Generating admin cert:

![ca](assets/admin-cert-gen.png)

For the other components like kube-scheduler, kube-controller-manager,
and kube-proxy the process is the same.

### How to use the generated certs?

![ca](assets/use-cert.png)

> **Note:** Because we are using a self-sign cert, we should pass cacert
too.

### Check certs expirations info

```bash
kubeadm certs check-expiration
```

[More info](https://kubernetes.io/docs/tasks/administer-cluster/kubeadm/kubeadm-certs/)


### Kubeconfig

![kubeconfig](assets/kubeconfig-1.png)

* `server` address and `certificates-authority` belongs to cluster
  section
* `client-key` and `client-certificate` place in user section
* Context illustrates which user should connect to which cluster
